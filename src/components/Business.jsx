...
const FeatureCard = ({icon,title,content,index})=>(
      <div className={`flex flex-row p-6 rounded-[10px] ${index===features.length-1? 'mb:0':'mb-6'}
       feature-card`}>
        <div className="w-[60px] h-[60px] rounded-full
          bg-dimBlue flex justify-center items-center">
...
const Business = () => {
  return (
    <section key={features} className={`${layout.section}`}>
      <div className={`${layout.sectionInfo}`}>
        <h2 className={`${style.heading2}`}>
         You do the business,<br className="xs:block hidden"/> we’ll handle the money.
        </h2>
        <p className={`${style.paragraph} mt-5 max-w-[570px]`}>
